...
{
	m_vertex_buffer_array.reset(new VertexBufferArray());
}
...
void TrianglesScene::OnCreate()
{
	...
	// bind the Vertex Array Object first, then bind and set vertex buffer(s), and then configure vertex attributes(s).
	m_vertex_buffer_array->AddVertexBuffer(m_vertex_buffer);
	m_vertex_buffer_array->SetIndexBuffer(m_index_buffer);
	this->InitShader();
...
void TrianglesScene::OnDraw()
{
	...
	m_vertex_buffer_array->Bind(); // bind VAO
	...
	glAssert(glDrawElements(GL_TRIANGLES, m_vertex_buffer_array->GetIndexBuffer()->GetCount(), GL_UNSIGNED_INT, nullptr));
